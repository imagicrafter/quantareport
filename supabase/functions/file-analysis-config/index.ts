...
/**
 * DEPRECATED: Edge function to provide webhook configuration for file analysis
 * This function has been replaced by n8n-webhook-proxy
 */
...
serve(async (req) => {
  ...
  try {
    ...
    console.warn("DEPRECATED FUNCTION ACCESSED: file-analysis-config - This function has been deprecated and will be removed. Please use n8n-webhook-proxy/config instead.");
    
    // Get environment from query parameters for logging
    const url = new URL(req.url);
    const env = url.searchParams.get("env") || "production";
    console.log(`Deprecated file-analysis-config accessed with env=${env}`);
