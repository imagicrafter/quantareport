...
BEGIN
  ...
  IF email_type = 'signup' THEN
    ...
    -- Clean up double slashes from the redirect_to URL, but preserve protocol slashes (e.g., https://)
    cleaned_redirect_to := regexp_replace(redirect_to, '(?<!:)/{2,}', '/', 'g');

    -- Construct the full verification link with the cleaned redirect URL
    confirmation_link := site_url || '/auth/v1/verify?token=' || token_hash || '&type=signup&redirect_to=' || cleaned_redirect_to;
