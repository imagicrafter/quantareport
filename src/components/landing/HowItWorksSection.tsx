...
const HowItWorksSection = () => {
  ...
  return (
    <section className="py-20 md:py-28 bg-gradient-to-b from-white to-quanta-gray/30">
            ...
            {steps.map((step, index) => (
              <div 
                ...
                className={`p-6 rounded-lg border transition-all duration-300 cursor-pointer ${
                  activeStep === index 
                    ? 'border-quanta-blue bg-quanta-blue/5 shadow-sm' 
                    : 'border-border hover:border-quanta-teal/50'
                }`}
                onClick={() => setActiveStep(index)}
              >
                <div className="flex items-start gap-4">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                    activeStep === index ? 'bg-quanta-blue/10' : 'bg-secondary'
                  }`}>
                    {step.icon}
                  </div>
                  <div>
